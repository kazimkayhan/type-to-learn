...
const RowDetail: React.FC<RowDetailProps> = ({ currentRowDetail, allRecords }) => {
  ...
  return (
    <div className="absolute inset-0 flex flex-col items-center justify-center px-3">
      <div className="my-card relative z-10 flex h-auto max-h-[90dvh] w-[min(26rem,calc(100vw-1.5rem))] min-w-0 select-text flex-col items-center justify-around overflow-y-auto rounded-2xl bg-white px-3 py-10 dark:bg-gray-900 sm:h-[32rem]">
        ...
        <div className="item flex flex-col gap-4">
          <div className="flex gap-6">
            <DataTag icon={ClockIcon} name="Avg. time" data={rowDetailData.time} />
            <DataTag icon={HashtagIcon} name="Sessions" data={rowDetailData.sumCount} />
          </div>
          <div className="flex gap-6">
            <DataTag icon={CheckCircle} name="Correct" data={rowDetailData.correctCount} />
            <DataTag icon={XCircle} name="Mistakes" data={rowDetailData.wrongCount} />
          </div>
        </div>
