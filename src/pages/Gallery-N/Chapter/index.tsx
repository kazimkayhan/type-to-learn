...
export default function Chapter({
  ...
  return (
    <div
      ref={ref}
      className="relative flex h-16 w-[calc(50%-0.375rem)] cursor-pointer flex-col items-start justify-center overflow-hidden rounded-xl bg-slate-100 px-3 py-2 dark:bg-slate-800 sm:w-40"
      onClick={() => onChange(index)}
    >
      <h1>Chapter {index + 1}</h1>
      <p className="pt-[2px] text-xs text-slate-600">
        {chapterStatus ? (chapterStatus.exerciseCount > 0 ? `${chapterStatus.exerciseCount} sessions` : 'Not practiced') : 'Loading...'}
      </p>
      {checked && (
