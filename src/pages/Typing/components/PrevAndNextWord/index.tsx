    ...
    <>
      {word ? (
        <Tooltip content={`Shortcut: ${shortCutKey}`}>
          <div
            onClick={onClickWord}
            className="flex max-w-[42vw] cursor-pointer select-none items-center text-gray-700 opacity-60 duration-200 ease-in-out hover:opacity-100 dark:text-gray-400 sm:max-w-xs"
          >
            ...
            <div className={`grow-1 flex w-full flex-col ${type === 'next' ? 'items-end text-right' : ''}`}>
              <p
                className={`font-mono text-base font-normal text-gray-700 dark:text-gray-400 sm:text-2xl ${
              ...
              {state.isTransVisible && (
                <p className="line-clamp-1 hidden max-w-full text-sm font-normal text-gray-600 dark:text-gray-500 sm:block">{word.trans.join('; ')}</p>
              )}
            </div>
